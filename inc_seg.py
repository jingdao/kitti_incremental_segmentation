...
import math
import sys
from architecture import MCPNet, PointNet
from class_util import classes, class_to_color_rgb
import itertools
...
#				obj_id = gt_obj_id[cluster_update[i]]
	...
	t = time.time() - t
	comp_time.append(t)
	print('Scan #%3d: %5d/%7d points acc %.3f time %.3f'%(count_msg, len(update_list), len(point_id_map), acc, t))
	count_msg += 1
	

GPU_INDEX = 0
MODEL_PATH = 'models/%s_model_%s.ckpt'%(net_type, VAL_AREA)
